package ui;

import javax.swing.*;
import javax.swing.border.EmptyBorder;
import java.awt.*;

public class BankRegisterFrame extends JFrame {

    private JTextField bankNameField, branchField,
            addressField, emailField, usernameField;

    private JPasswordField passwordField;
    private JComboBox<String> bankTypeCombo;

    public BankRegisterFrame() {

        setTitle("Bank Registration");
        setExtendedState(JFrame.MAXIMIZED_BOTH);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

        JPanel backgroundPanel = new JPanel(new GridBagLayout());
        backgroundPanel.setBackground(new Color(235, 242, 250));

        JPanel card = new JPanel();
        card.setPreferredSize(new Dimension(850, 650));
        card.setBackground(Color.WHITE);
        card.setLayout(new GridBagLayout());

        card.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(
                        new Color(220,220,220),1),
                new EmptyBorder(25,25,25,25)
        ));

        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(12,12,12,12);
        gbc.fill = GridBagConstraints.HORIZONTAL;

        // Title
        JLabel title = new JLabel("BANK REGISTRATION");
        title.setHorizontalAlignment(SwingConstants.CENTER);
        title.setFont(new Font("Segoe UI", Font.BOLD, 28));
        title.setForeground(new Color(25,118,210));

        gbc.gridx = 0;
        gbc.gridy = 0;
        gbc.gridwidth = 2;

        card.add(title, gbc);

        gbc.gridwidth = 1;

        // Bank Name
        gbc.gridx = 0;
        gbc.gridy = 1;
        card.add(createLabel("Bank Name"), gbc);

        bankNameField = createTextField();
        gbc.gridx = 1;
        card.add(bankNameField, gbc);

        // Branch Name
        gbc.gridx = 0;
        gbc.gridy = 2;
        card.add(createLabel("Branch Name"), gbc);

        branchField = createTextField();
        gbc.gridx = 1;
        card.add(branchField, gbc);

        // Bank Type
        gbc.gridx = 0;
        gbc.gridy = 3;
        card.add(createLabel("Bank Type"), gbc);

        bankTypeCombo = new JComboBox<>(
                new String[]{
                        "Public",
                        "Private",
                        "Cooperative"
                });

        bankTypeCombo.setFont(
                new Font("Segoe UI", Font.PLAIN, 18));

        bankTypeCombo.setPreferredSize(
                new Dimension(350,45));

        gbc.gridx = 1;
        card.add(bankTypeCombo, gbc);

        // Address
        gbc.gridx = 0;
        gbc.gridy = 4;
        card.add(createLabel("Address"), gbc);

        addressField = createTextField();
        gbc.gridx = 1;
        card.add(addressField, gbc);

        // Email
        gbc.gridx = 0;
        gbc.gridy = 5;
        card.add(createLabel("Email"), gbc);

        emailField = createTextField();
        gbc.gridx = 1;
        card.add(emailField, gbc);

        // Username
        gbc.gridx = 0;
        gbc.gridy = 6;
        card.add(createLabel("Username"), gbc);

        usernameField = createTextField();
        gbc.gridx = 1;
        card.add(usernameField, gbc);

        // Password
        gbc.gridx = 0;
        gbc.gridy = 7;
        card.add(createLabel("Password"), gbc);

        passwordField = new JPasswordField();
        passwordField.setFont(
                new Font("Segoe UI", Font.PLAIN, 18));
        passwordField.setPreferredSize(
                new Dimension(350,45));

        gbc.gridx = 1;
        card.add(passwordField, gbc);

        // Buttons
        JButton registerBtn = new JButton("REGISTER");
        styleButton(registerBtn,
                new Color(46,125,50));

        JButton backBtn = new JButton("BACK");
        styleButton(backBtn,
                new Color(211,47,47));

        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.CENTER,20,5));
        buttonPanel.setBackground(Color.WHITE);
        buttonPanel.add(registerBtn);
        buttonPanel.add(backBtn);

        gbc.gridx = 0;
        gbc.gridy = 8;
        gbc.gridwidth = 2;
        card.add(registerBtn, gbc);
        gbc.gridy =9;
        card.add(backBtn,gbc);
        backgroundPanel.add(card);
        add(backgroundPanel);

        registerBtn.addActionListener(e -> registerBank());

        backBtn.addActionListener(e -> {
            dispose();
            new LoginFrame("BANK");
        });

        setVisible(true);
    }

    private JLabel createLabel(String text) {
        JLabel label = new JLabel(text);
        label.setFont(
                new Font("Segoe UI", Font.BOLD, 18));
        return label;
    }

    private JTextField createTextField() {
        JTextField field = new JTextField();
        field.setFont(
                new Font("Segoe UI", Font.PLAIN, 18));
        field.setPreferredSize(
                new Dimension(350,45));
        return field;
    }

    private void styleButton(JButton button, Color color) {

        button.setBackground(color);
        button.setForeground(Color.WHITE);

        button.setFont(
                new Font("Segoe UI", Font.BOLD, 18));

        button.setPreferredSize(
                new Dimension(180,50));

        button.setFocusPainted(false);
    }

    private void registerBank() {

        String bankName = bankNameField.getText().trim();
        String branchName = branchField.getText().trim();

        String bankType =
                bankTypeCombo.getSelectedItem().toString();

        String address = addressField.getText().trim();
        String email = emailField.getText().trim();
        String username = usernameField.getText().trim();

        String password =
                new String(passwordField.getPassword());

        if(bankName.isEmpty() || branchName.isEmpty()
                || address.isEmpty() || email.isEmpty()
                || username.isEmpty() || password.isEmpty()) {

            JOptionPane.showMessageDialog(
                    this,
                    "Please fill all fields"
            );
            return;
        }

        JOptionPane.showMessageDialog(
                this,
                "Bank Registered Successfully!"
        );

        clearFields();
    }

    private void clearFields() {

        bankNameField.setText("");
        branchField.setText("");
        addressField.setText("");
        emailField.setText("");
        usernameField.setText("");
        passwordField.setText("");

        bankTypeCombo.setSelectedIndex(0);
    }
}